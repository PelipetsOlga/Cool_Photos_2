package com.mobapply.happymoments;


public final class Constants {
    public static final String APP_FOLDER="HappyMoments";

    public static final String PREFS_NAME = "HappyMoments";
    public static final String PERIOD_UPDATING = "period";
    public static final String SHUFFLE = "shuffle";
    public static final String MODE_CONSCIOUS = "mode_conscious";
    public static final String FIRST_START = "first_start";

    public static final boolean DEFAULT_SHUFFLE = false;
    public static final boolean DEFAULT_MODE_CONSCIOUS = true;

    public static final int DEFAULT_PERIOD_UPDATING = 1;
    public static final long SHOW_TIME=33;

    public static final long SHOW_TIME_CONSCIOUS=2000;
    public static final long SHOW_TIME_SUBCONSCIOUS=33;

    public static final String EXTRA_ID="extra_id";
    public static final String EXTRA_COUNT="extra_count";
    public static final String EXTRA_TITLE="extra_title";
    public static final String EXTRA_FILE_NAME="extra_file_name";
    public static final String EXTRA_STOP_SERVICE="extra_stop_service";
    public static final String EXTRA_ADD_PICTURE="extra_add_picture";

    public static final int REQUEST_CODE_PHOTO=33;
    public static final int REQUEST_CODE_GALLERY=34;

}
